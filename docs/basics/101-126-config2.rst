...
Other than with configuration files, the behavior of Git, Git-annex, and DataLad can
also be altered with environment variables, providing a configuration method entirely
independent from the :command:`git config` command.
Usually, every option that can be set in a configuration file also has an environment
variable. If the environment variable is set, it takes precedence over options set in
configuration files. For example, the ``user.name`` configuration of Git can be overridden
by its associated environment variable, ``GIT_AUTHOR_NAME``.
...
.. [#f4] **Some more on environment variables**:
         ...
         variable: To find the value of the environment variable ``HOME`` for example you would
