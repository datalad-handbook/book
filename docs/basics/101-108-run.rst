...
.. runrecord:: _examples/DL-101-108-101
   :language: console
   :workdir: dl-101
   :caption: its impossible to remember how a large data analysis produced which result for a human, but datalad can help to keep track. To see this in action, we'll do a data analysis. Start with yoda principles and structure ds with code directory.
...
.. runrecord:: _examples/DL-101-108-102
   ...
   :caption: We will create a script to execute. Let's make one that summarizes the podcasts titles in the longnow dataset:
   :cast: 02_reproducible_execution
...
.. runrecord:: _examples/DL-101-108-107
   :language: console
   :workdir: dl-101/DataLad-101
   :caption: A run command that does not result in changes (no modifcations, no additional files) will not produce a record in the dataset history. So what happens if we do the same again?
   :cast: 02_reproducible_execution
