...
.. windows-wit:: please use datalad diff --from main --to HEAD~1
   ...
   The ``--from`` argument specifies a different starting point for the comparison - the ``main`` or :term:`main` :term:`branch`, which would be the starting point on most Unix-based systems.
