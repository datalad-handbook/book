...
During the second step, ``hirni`` relies completely on conversion-relevant
...
.. findoutmore:: On some basics of DICOMs
   ...
   *image data sets*. The information within the header is organized
   ...
   data from several *MRI sequences*: A typical MRI study collects anatomical
   images (T1 weighted), and study-specific data such as task-based and/or
   resting state fMRI data, `diffusion-weighted MRI data <https://en.wikipedia.org/wiki/Diffusion_MRI>`_,
   `MR angiography <https://en.wikipedia.org/wiki/Magnetic_resonance_angiography>`_,
   or other `sequences <https://en.wikipedia.org/wiki/MRI_sequence>`_ in a single
   acquisition. All DICOM files that belong to the same sequence are called
   ...
   single subject. During conversion, each image series needs to be identified,
...
.. findoutmore:: What's inside the toolbox?
   ...
   to perform for you. For this, it references a number of potentially relevant
   ...
   will retrieved automatically if needed [#f3]_.
...
Even if you don't have DICOM data yet, you can already curate metadata about your
study into the study dataset. This helps to capture information that you now know
from the top of your head, but that you would later need to dig through old documents
for to find out about. You can, however, also get to this step at any later point
in time.
...
:command:`datalad save` afterwards, or use ``hirni``\s webUI for assistance.
...
To get to this structure, DICOMs are imported with the :command:`datalad hirni-import-dcm`
...
that this subject should be identified with with the ``--anon-subject`` option.
Note that some information can be inferred from DICOM headers if
it was correctly filled in during scanning, for example the acquisition ID and
the subject identifier.

To import the first acquisition of the first subject, your command may look as
follows::
...
.. findoutmore:: Internals of this command
   ...
   Ultimately, DICOMs are only extracted from the tarball archive to
   record their headers' metadata. For this metadata extraction, the ``dicom``
   ...
   data within the acquisition's subdataset.
   Based on this file ...
   ...
   ``master`` could be retrieved from the tarball. Because this ensures one available
   source for the data, the file content of the DICOMs in ``master`` can be dropped
