...
Take that checksum and paste it after ``datalad rerun`` (The first 6-8 characters
of the checksum would be sufficient).
...
We can see that a new ``datalad run`` was recorded. In the datasets history, this action is
