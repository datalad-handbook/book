...
.. index::
   single: datalad command (in book); rerun
...
:term:`datalad rerun` command! That's it!", he says happily.
...
.. index::
   single: datalad command (in book); diff

Two cool tools that go beyond the ``git log``
are the :term:`datalad diff` and ``git diff`` commands.
