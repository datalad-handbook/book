...
:windows-wit:`on an alternative <ww-heredoc-alt2>`.

.. windows-wit:: Here's a script adjustment for Windows users
   :name: ww-heredoc-alt2
   :float: tb
...
Check the :windows-wit:`on a command variant <ww-diff-adjusted>`.
...
Check the :windows-wit:`on a command variant <ww-log-adjusted>`.
