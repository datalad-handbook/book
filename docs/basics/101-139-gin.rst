...
Afterwards, you can publish your dataset with :command:`datalad publish`. As the
...
(that records the source of the subdaset), and the ``midterm_project`` dataset
...
:command:`datalad subdatasets` [#f2]_ (replace the URL shown here with the URL
...
a location of your choice, and modify the ``.gitmodule`` entry accordingly.
...
.. [#f2] Alternatively, you can configure the siblings url with :command:`git config`::
