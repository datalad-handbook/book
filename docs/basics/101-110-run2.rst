...
locally after a clone, and we did not :command:`datalad get` it yet!
...
dataset: With a cloned dataset one can very simply rerun :command:`datalad run` commands
...
.. findoutmore:: But what if I have a lot of outputs?
   ...
   ``-o directory/*``. And, just as for ``-i``/``--input``, you could use
