...
If this happens, you can get out of ``git log`` by pressing ``q``. For Windows,
also see :windows-wit:`on showing history <ww-adjustedmode-log>`.
...
.. windows-wit:: Your Git log may be more extensive - use 'git log main' instead!
   :name: ww-adjustedmode-log
   :float: tb

   The output of :gitcmd:`log` shown in this book and the output you will see in your own datasets when executing the same commands may not always match -- many times you might see commits about a "git-annex adjusted branch" in your history.
   ...
   This should display the same output that this book shows.
   ...
   With this workaround, you will be able to display the dataset history from the same branch that this book and all other operating system display.
   Thus, whenever a code snippet contains a line that starts with ``git log``, copy it and append the term ``main`` or ``master``, whichever is appropriate.

   If you are eager to help to improve the online-handbook, you could do us a favor by reporting any places with mismatches between Git logs on Windows and those in the handbook.
...
.. gitusernote:: Create internals
   ...
   **For non-Git-Users:** these hidden *dot-directories* and *dot-files* are necessary for all Git operations. Please do not tamper with them, and, importantly, **do not delete them!**
