...
.. windows-wit:: Good news! Here is something that is easier on Windows
   ...
   This section describes something that is very confusing for people that have just started with DataLad: Some files in a dataset can't be modified, and if one tries, it results in a "permission denied" error.
   Why is that?
   ...
   Just be mindful that you will not encounter the errors that this book shows next.
