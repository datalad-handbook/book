...
We further learned that when we modified content in ``notes.txt``, the modified content was
...
they just point to the place where the actual file content secretly resides.
...
When a file is saved into a dataset to be tracked, DataLad gives this file to Git-annex
if it

#. is larger than a certain (configurable) size

#. and/or not of certain specified types (e.g. textfiles, as with the
...
This process is often referred to as a file being *annexed*.

For a demonstration that this file path is not complete gibberish,
take the target path of any of the books symlinks and
...
.. container:: toggle
   ...
   have identical contents, or whether file content changed).
   ...
   operating systems need this information. The key is also one of the subdirectory
   names in the path. This two-level structure is implemented because it helps to prevent
   ...
   and their sole purpose to exist is to avoid issues with too many files in one directory.