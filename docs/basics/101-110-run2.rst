...
.. find-out-more:: But what if I have a lot of outputs?
   ...
   To glob for files in multiple levels of directories, set one ``*`` per level, for example ``-o directory/*/*.dat`` when operating with DataLad version ``<=0.15.4``.
   If your version of DataLad is more recent than that, use ``**`` (a so-called `globstar <https://www.linuxjournal.com/content/globstar-new-bash-globbing-option>`_) for a recursive glob through any number directories.
