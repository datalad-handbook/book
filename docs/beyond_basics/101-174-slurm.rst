...
1. Critical conflict: A git repository must not be accessed concurrently
      ...
      * Concurrent git calls are race conditions == undefined behavior. Imagine conflicting git checkout calls in the same clone ...
...
3. Breaking machine-actionable reproducibility: Should the SLURM job script be part of the rerun record?
      ...
      * If the ``datalad run ...`` call was inside the SLURM job script then it would need to be modified to rerun from ``datalad run <command>`` to ``datalad rerun <commithash>``. Thus is cannot be reused unmodified.
