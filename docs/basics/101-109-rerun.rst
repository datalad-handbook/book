...
.. windowsworkarounds:: please use datalad diff --from master --to HEAD~1
   ...
   For now, to get the same output as shown in the code snippet below, use the following command::

      datalad diff --from master --to HEAD~1

   The ``--from`` argument specifies a different starting point for the comparison - the :term:`master` :term:`branch`, which would be the starting point on most Unix-based systems.
...
.. windowsworkarounds:: use "git log master -- recordings/podcasts.tsv"
   ...
   Please append ``master`` to ``git log``, prior to the file specification.
