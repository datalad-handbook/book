...
later section on YODA principles (todo: link): Collecting scripts
that works with content of a subdataset outside of this
subdataset, in a dedicated ``code/`` directory, and collating the
output of the execution outside of the subdataset as well -- and
therefore to not modify the subdataset.

The motivation behind this
will become clear in later sections, for now we'll start with best-
practice building. Therefore, create a subdirectory ``code/`` in the ``DataLad-101``
superdataset:
...
It will loop through the file names and
