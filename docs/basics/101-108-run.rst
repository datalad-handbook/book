...
.. runrecord:: _examples/DL-101-108-104
   ...
   $ datalad save -m "Add simple script to write a list of podcast speakers and titles" code/list_titles.sh
...
.. index::
   single: datalad command (in book); run

The :term:`datalad run` command can help with this. It records a command's impact on a
dataset.
...
.. container:: toggle
   ...
   .. container:: header
      ...
      **Addition: Why is there a "notneeded" in the command summary?**

   If you have stumbled across the command execution summary
   ``save (notneeded: 1, ok: 1)`` and wondered what is "notneeded":
   the ``datalad save`` at the end of a ``datalad run`` will query all potential
   ...
   a "notneeded" summary. Thus, after a ``datalad run``, you'll get a
