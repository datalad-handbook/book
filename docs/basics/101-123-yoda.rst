...
As you will see, complying to these principles is easy if you you
...
This, for example, would be a directory structure of a very comprehensive [#f3]_
...
Having input data as independent components that are not influenced (only
...
Later you might also write a scientific paper about your analysis.
This whole dataset can easily become a modular component in a paper
...
It is usually costly to collect data, and economically unfeasible to keep
...
even easier: DataLads ``yoda`` procedure. Just as the ``text2git`` procedure
...
#. configured everything in the ``code/`` directory to be tracked by Git, not Git-annex [#f4]_
...
.. [#f4] To re-read how ``.gitattributes`` work, go back to section :ref:`config`, and to remind yourself
