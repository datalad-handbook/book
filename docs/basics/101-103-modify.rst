.. index::
   pair: modify dataset; with DataLad
.. _modify:
...
Check the :find-out-more:`on some heredocs background <fom-heredoc>`.
...
.. find-out-more:: How does a heredoc (here-document) work?
   ...
   *delimiting identifiers*. Here, these identifiers are *EOT* (short
   ...
   you create and modify a ``.txt`` file while working with this book.

Running this command will create ``notes.txt`` in the
root of your ``DataLad-101`` dataset, but see the :windows-wit:`on terminal compatibility <ww-heredoc>`:
...
Check the :find-out-more:`on 'git log' for even more possibilities <fom-log-options>`.
