...
user (``--global``) [#f3]_::
   ...
   $ FIXME git config --global --add datalad.result-hook.readme.call-json 'run {{"cmd":"cp ~/templates/standard-readme.txt {path}/README", "dataset":"{dsarg}","explicit":true}}'
   $ FIXME git config --global --add datalad.result-hook.readme.match-json '{"type": "dataset","action":"create","status":"ok"}'

Here is what this writes to the ``~/.gitconfig`` file::

    [datalad "result-hook.readme"]
    FIXME        call-json = run {{\"cmd\":\"cp ~/templates/standard-readme.txt {path}/README\", \"dataset\":\"{dsarg}\",\"explicit\":true}}
    FIXME        match-json = {\"type\": \"dataset\",\"action\":\"create\",\"status\":\"ok\"}
