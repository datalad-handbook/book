...
and  `dcm2niix <https://github.com/rordenlab/dcm2niix>`_, which is used by
``HeuDiConv`` internally. Find out more about these tools in the hidden section
below.
...
.. findoutmore:: Underlying tools for DICOM conversions
   ...
   ``hirni``\s aim is to ease and automate the use of these tools.
   ...
   **HeuDiConv**: ``HeuDiConv`` is a tool to convert and structure DICOM data.
   ...
   ``HeuDiConv`` can help to see how ``hirni`` makes conversions from raw data to
   BIDS datasets easier.
